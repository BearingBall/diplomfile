...
def general_objectness(labels, local_rank):  # as in InvisibleCloak
    score = torch.tensor(0.0).to(f'cuda:{local_rank}')
    for i in range(len(labels["labels"])):
        # and labels["scores"][i] > 0.6
        if labels["labels"][i] == 1:
            score += max(labels["scores"][i] + torch.tensor(1.0).to(f'cuda:{local_rank}'), torch.tensor(0.0).to(f'cuda:{local_rank}'))**2
    return score
