...
def adversarial_loss_function(predict, patch, device, tv_scale):
    return metrics.general_objectness(predict, device) + tv_scale * metrics.total_variation(patch) + metrics.printability_loss(patch, device)
    ...
    for val_idx, (images, labels, img_ids, scale_factor) in enumerate(val_loader):
        ...
        if augmented_patch is not None:
            for i, _ in enumerate(images):
                for label in labels[i]:
                    attacked_images[i] = attacked_images[i]#insert_patch(attacked_images[i], augmented_patch, label, 0.4, device, True)
