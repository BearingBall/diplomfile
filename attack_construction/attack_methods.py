...
def training_step(model, patch, augmentations, images, labels, loss, device, optimizer):
    ...
    for i, image in enumerate(images):
        if labels[i][0][2] * labels[i][0][3] != 0:
            ...
            if (len(lbl) != 0):
                targets.append({'boxes': torch.stack(lbl), 'labels': torch.ones(len(lbl), dtype=torch.int64).to(device)})
    ...
    if len(input) != 0:
        ...
        #cost = sum(costs)
        #print(predict)
        cost = -1 * predict['classification'] + -1 * predict['bbox_regression'] + 0.0001 * metrics.total_variation(patch)
