...
def training_step(model, patch, augmentations, images, labels, loss, device, grad_rate):
    ...
    predict = model(attacked_image)

    costs = loss(predict, patch, device)

    grad = torch.autograd.grad(sum(outputs=predict[0]['scores']), inputs=patch, retain_graph=True, create_graph=True, allow_unused=True)[0]
